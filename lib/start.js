...
export async function start(userOptions = {}) {
  ...
  const browserFile = options.browser === 'folder' ? 'data-browser-folder.js'
    ...
    : 'data-browser.js';
  // JSPOD_BROWSER_URL overrides the pin — e.g. to keep the browser on the
  // last published version while running an unpublished jspod from a checkout
  // (the jsdelivr asset for an unpublished version would 404).
  const dataBrowserUrl = process.env.JSPOD_BROWSER_URL ||
    `https://cdn.jsdelivr.net/npm/jspod@${pkg.version}/${browserFile}`;
  ...
  // Mount JSS app plugins (jss >=0.0.216 --plugin <module[@prefix]>, repeatable).
  // Relative module paths are resolved against jspod's cwd so pm2/systemd
  // callers get the same plugin regardless of exec cwd.
  for (const p of options.plugins || []) {
    const [mod, ...prefix] = p.split('@');
    const resolved = mod.startsWith('/') ? p : [resolve(mod), ...prefix].join('@');
    jssArgs.push('--plugin', resolved);
  }
